...
def clean_status_data(file_path):
    ...
    with open(clean_file_path, 'w') as f_clean:
        ...
        for order_uuid, status in status_data.items():
            payment_status = ('success' if status['success'] is True
                              else 'failure')
...
def calculate_age(birth_date, datetime_format):
    ...
    now = datetime.now()
    datetime_diff = now - datetime.strptime(birth_date, datetime_format)
    age = datetime_diff // timedelta(days=DAYS_IN_YEAR)
    return age
...
def clean_customers_data(file_path):
    ...
    with open(file_path) as f:
        ...
        with open(clean_file_path, 'w') as f_clean:
            ...
            for line in reader:
                line = {key: clean_field(line[key]) for key in line}
                line['age'] = calculate_age(line['birth_date'],
                                            datetime_format)
                writer.writerow(line)
...
def create_table(cur, table_name, table_cols, constraint_cols=None):
    ...
    if constraint_cols is None:
        ...
        sql_text = 'CREATE TABLE IF NOT EXISTS {} ({}, CONSTRAINT hw4_unq_rec UNIQUE ({}))'
        sql_query = sql.SQL(sql_text).format(table_id, columns_list,
                                             constraints_list)
...
(process_orders_task >> process_status_task >> process_customers_task
 >> process_goods_task >> create_dataset_task)
